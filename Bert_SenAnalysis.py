...
class Bert_SenAnalysis(BertPreTrainedModel):
    ...
    def acc_rec_f1(self, preds, labels):
        ...
        #accuracy_score(y_true=labels, y_pred=preds)  # simple_accuracy(preds, labels)
    ...
    def predict(self, bert_encode):
        ...
        if np.argmax(bert_encode, axis=1).any() not in [0,1,2]:
            print('attention:',np.argmax(bert_encode, axis=1))
        return np.argmax(bert_encode, axis=1)#按行取最大值，返回最大值所在的索引
