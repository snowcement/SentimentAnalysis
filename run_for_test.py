...
# TODO: 分段特征
# def convert_doc_to_feature(doc, max_seq_length, split_num, cls_token_at_end=False, pad_on_left=False,
...
# def predict_test(examples, model, device):
#     '''
#     根据测试文本生成标注结果
#     :param doc:带预测语料
#     :return:标注结果
#     '''
#     features = convert_doc_to_feature(examples, args.max_seq_length, split_num=args.split_num,
...
def preprocess():
    ...
    for input_ids, input_mask, segment_ids in dataloader:
        ...
        with torch.no_grad():
            logits = model(input_ids=input_ids, token_type_ids=segment_ids,
                           attention_mask=input_mask)[0]#.detach().cpu().numpy()
    ...
    # df.to_csv('./data/test_final.csv', sep=',', encoding='utf_8_sig', header=True, index=False)
