#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2019/10/23 20:22
# @Author  : wutt
# @File    : progress_util.py
...
class ProgressBar(object):
    ...
    # 显示函数，根据当前的处理进度i显示进度
    # 效果为[>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>]100.00%
    def show_process(self, train_acc, train_rec, train_loss, f1, used_time, i):
        ...
        process_bar =  '%d'%num_steps + '/' + '%d'%self.epoch_size + '[' + '>' * num_arrow + '-' * num_line + ']'\
                      + '%.2f' % percent + '%' + ' - train_acc ' + '%.4f'%train_acc + ' - train_rec ' + '%.4f'%train_rec + ' - train_loss '+ \
